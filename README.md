# FGO 最优羁绊组队求解器 · Local Box

本项目以 [GPL-3.0](LICENSE) 协议开源。

本仓库是 [pikaball/fgo-calc](https://github.com/pikaball/fgo-calc) 的本地增强 fork。核心组合求解思路与原项目来自上游；本 fork 在当前上游基础上增加个人 Box、长期羁绊 Planner、FGO/Stream 本地账户快照导入、冠位戴冠战 8 加成位与自动数据同步等功能。
...
- 现已支持助战、活动个体加成，以及部分活动从者入队时提供的全队羁绊加成。

- 绊15「夢火の導き」分为两种状态：已达当前上限者自身收益为0但继续提供全队+25%；已开放更高上限（例如15→16）的从者自身仍正常获得羁绊，同时继续提供+25%。多个来源可叠加，并与礼装/活动百分比在同一加算桶内计算。
...
## Local Box fork 的额外求解能力

- **个人 Box / 礼装库存约束**：只从实际持有的从者与自备羁绊礼装中求解。
- **长期培养 Planner**：区分司机 / 普通 / 老板、固定出场、培养目标与手动优先级；提供单场最大、长期均衡、优先收尾三种目标。培养评分只影响排序，不伪装成游戏实际羁绊。
- **冠位戴冠战 8 加成位**：自备普通礼装5张 + 自家 Grand 额外0 Cost报酬礼装1张 + 助战 Grand 两张礼装。
- **精确逐从者收益**：结果显示每骑实际吃到的百分比、固定加成、夢火来源与真实本场羁绊。
- **全队活动加成 provider**：只有 provider 实际入队时才对队伍生效，可与夢火等百分比正确叠加。
- **显式 include 优先**：必选从者/礼装不会因同时存在于排除集合而被静默丢弃。
